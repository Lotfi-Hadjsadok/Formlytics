...
import { Eye, Download, Search, Filter, Settings, ArrowUpDown, ArrowUp, ArrowDown, GripVertical } from "lucide-react"
...
export function FormEntriesTable({ entries, formFields }: FormEntriesTableProps) {
  ...
  const { columns, setColumns, resetColumnOrder } = useColumnOrder(formFields)
  ...
  const filteredAndSortedEntries = useMemo(() => {
    let filtered = entries.filter(entry => {
      ...
      if (searchTerm) {
        ...
        const matchesSearch = Object.values(entry.answers).some(answer => 
          String(answer).toLowerCase().includes(searchLower)
        ) || format(new Date(entry.createdAt), 'MMM dd, yyyy HH:mm').toLowerCase().includes(searchLower)
      ...
      for (const [fieldId, filterValue] of Object.entries(fieldFilters)) {
        if (filterValue && filterValue !== 'all') {
          const answer = entry.answers[fieldId]
          if (!answer || !String(answer).toLowerCase().includes(filterValue.toLowerCase())) {
            return false
          }
        }
      }
    ...
    return filtered
  }, [entries, searchTerm, sortConfig, dateFilter, fieldFilters])
      ...
      <div className="flex justify-between items-center">
        <div className="flex items-center gap-4">
          <h4 className="text-sm font-medium text-gray-700">
            {filteredAndSortedEntries.length} of {entries.length} response{entries.length !== 1 ? 's' : ''}
          </h4>
          {searchTerm && (
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <span className="text-sm text-gray-500">
              Showing {((currentPage - 1) * pageSize) + 1} to {Math.min(currentPage * pageSize, filteredAndSortedEntries.length)} of {filteredAndSortedEntries.length} entries
            </span>
